...
class DFS(rows: Int, cols: Int) {
    ...
    private suspend fun dfs(
        ...
        for (i in 0..3) {
            ...
            if (isValid(startX + dx[i], startY + dy[i], path, board)) {
                if (1 + dist[startX][startY] < dist[startX + dx[i]][startY + dy[i]]) {
                    board[startY + dy[i]][startX + dx[i]] = Finder.EXPLORE_HEAD_CELL_CODE
                    delay(speed)
                    when (i) {
                        0 -> {
                    ...
                    if ((startX + dx[i] == endX) && (startY + dy[i] == endY)) {
                        board[startY + dy[i]][startX + dx[i]] = Finder.EXPLORE_CELL_CODE
                        return true
                    }
        ...
        board[startY][startX] = Finder.END_CELL_CODE
        delay(15)
        board[startY][startX] = Finder.EXPLORE_CELL_CODE
        return false
    }
    ...
    private fun isValid(
        ...
        if (i >= 1 && j >= 1 && i <= 15 && j <= 15 && path[j][i] == 'O' && board[j][i] != Finder.OBSTACLE_CELL_CODE) {
            return true
        }
        return false
    }
